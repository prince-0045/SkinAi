"""
EfficientNetB3 — Skin Disease Classification (Google Colab)
===========================================================
...
Outputs (saved to `trained_artifacts/`):
    EfficientNetB3.h5          — full Keras model
    EfficientNetB3.pkl         — metadata + pickled model bytes
    EfficientNetB3_confusion_matrix.png
    EfficientNetB3_training_curves.png
    artifact_index.json
"""
...
# ── Model-specific hyperparameters (tuned for EfficientNetB3) ──
MODEL_NAME = "EfficientNetB3"
IMAGE_SIZE = 300          # EfficientNetB3 native resolution for best accuracy
BATCH_SIZE = 16           # 16 fits comfortably in Colab T4 GPU
VAL_SPLIT = 0.2
DROPOUT_RATE = 0.40       # slightly higher dropout — B3 is larger so benefits
HEAD_EPOCHS = 10          # more epochs for the head stage
FINE_TUNE_EPOCHS = 15     # generous fine-tuning budget; early-stop will cut
INITIAL_LR = 1e-3
FINE_TUNE_LR = 5e-6       # lower LR for larger backbone
FINE_TUNE_LAST_N = 60     # unfreeze more layers — B3 has ~354 layers total
...
def print_data_overview():
    if not TRAIN_DIR.exists() or not TEST_DIR.exists():
        raise FileNotFoundError(
            "Expected SkinDisease/train and SkinDisease/test directories."
        )
    ...
    if set(train_counts) != set(test_counts):
        print("⚠️  Train and test class names do NOT match exactly.")
    else:
...
def build_efficientnetb3(num_classes: int):
    ...
    x = layers.RandomZoom(0.12, seed=SEED, name="aug_zoom")(x)
    x = layers.RandomContrast(0.10, seed=SEED, name="aug_contrast")(x)

    # Preprocessing
    x = layers.Lambda(lambda t: tf.cast(t, tf.float32), name="cast_float32")(x)
    x = layers.Lambda(
        keras.applications.efficientnet.preprocess_input, name="preprocess"
    )(x)
    ...
    # Classification head — slightly beefier for B3
    x = layers.BatchNormalization(name="head_bn")(x)
    x = layers.Dense(256, activation="relu", name="head_dense")(x)
    x = layers.Dropout(DROPOUT_RATE, name="head_dropout")(x)
    outputs = layers.Dense(num_classes, activation="softmax", name="predictions")(x)

    model = keras.Model(inputs=inputs, outputs=outputs, name="EfficientNetB3_classifier")
    return model, base


model, base_model = build_efficientnetb3(NUM_CLASSES)
model.summary()
